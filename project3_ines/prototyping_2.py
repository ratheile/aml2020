...
# Find peaks
ecg_orig_clean = nk.ecg_clean(crv, sampling_rate=sample_rate)
...
fig
# %% Experiments from Ines
ecg_orig_clean = nk.ecg_clean(crv, sampling_rate=sample_rate)
heartbeats = nk.ecg_segment(ecg_orig_clean, rpeaks=None, sampling_rate=sample_rate, show=True)
# %%
heartbeats.keys()
print(len(heartbeats['1']))
print(len(heartbeats['18']))
# %%
newDict = dict()
# Iterate over all the items in dictionary and filter items which has even keys
for (key, value) in heartbeats.items():
   # Check if key is even then add pair to new dictionary
  newDict[key] = value['Signal']
# %%
