...
#%% PCA to reduce to 10 dimensions (Gaussian processes loose efficiency with high dimensions)
# https://scikit-learn.org/stable/modules/gaussian_process.html#gaussian-process
pca_decomp = 1
if pca_decomp == 1:
  pca = decomposition.PCA(n_components=3)
  colname = ['pca1','pca2','pca3']
  pca.fit(X_red)
  X_red_pca = pca.transform(X_red)
  X_red = pd.DataFrame(data=X_red_pca, columns=colname)
...
classifiers={
  'gpclf': lambda: GaussianProcessClassifier(
      kernel=1.0 * RBF(1.0),
...
if pca_decomp == 1:
   ...
   X_test_pca = pd.DataFrame(data=X_test_pca, columns=colname)
...
submissions.to_csv(f'{repopath}/prototyping/submission_gpclf_pca.csv', index=False)
