...
classifiers={
  ...
  'SVC': lambda: SVC(
    ...
    tol=0.001, 
    #class_weight='balanced',
    class_weight={0: weights[0], 1: weights[1]*1.5, 2: weights[2]},
    verbose=True, 
    max_iter=-1, # no limit
...
clf = OneVsOneClassifier(classifiers['lightgbm']()).fit(X_train,y_train)
...
submissions.to_csv(f'{repopath}/prototyping/submission_lightgbm_ovo.csv', index=False)
